# Copyright Compendium Documentation

Welcome to the documentation for the Copyright Compendium web display project.

## Overview

This repository contains a simple web display for the U.S. Copyright Office's Copyright Compendium. The Compendium is available as a PDF on Copyright.gov, but is difficult to navigate. This project provides a navigable and searchable web version.

## Technology Stack

- **Frontend**: TypeScript with Vite build system
- **UI Framework**: USWDS (U.S. Web Design System) components
- **Search**: Algolia search with autocomplete
- **Testing**: Vitest with comprehensive unit and integration tests
- **Type Checking**: TypeScript with strict mode enabled

## Documentation Sections

### Quality Assurance
- [QA Overview](qa.md) - Quality assurance methodology and tools
- [Testing](testing.md) - Automated content checking QA engine
- [UI Testing](ui-testing.md) - Frontend testing with Vitest
- [Translation Testing](testing-translation.md) - Translation feature testing
- [PDF Text Tests](pdf-text-tests.md) - PDF to HTML text comparison tests
- [PDF to HTML Comparison](compare-pdf-html.md) - Comparison methodology

### Features
- [Translation Feature](translation-feature.md) - Browser-based translation overview
- [Translation Implementation](translation-implementation.md) - Implementation guide
- [Translation Fix Summary](translation-fix-summary.md) - Recent fixes and updates

### Reports
- [Broken Links Report](broken-links-report.md) - Link validation report
- [Audit Summary](audit-summary.md) - Comprehensive audit results

## Quick Links

- [Main Repository README](https://github.com/adhocteam/copyright-compendium)
- [Live Site](https://copyright-compendium.vercel.app)
- [GitHub Repository](https://github.com/adhocteam/copyright-compendium)

## Getting Started

For information on running the project locally, building, and deploying, please refer to the [main README.md](https://github.com/adhocteam/copyright-compendium/blob/main/README.md) in the repository root.
